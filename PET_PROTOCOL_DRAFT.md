...
```json
{
  ...
  "personality_card": "一只慢热但好奇的小猫，喜欢在屏幕边缘观察。",
	...
	`interaction_capabilities` 声明这只宠物作为来访宠物时愿意/能够接受哪些互动。宿主 Runtime 只展示自己支持、宠物也声明支持、且宿主规则允许的交集。旧宠物没有声明时，默认只支持 `petting`、`message` 和 `return_home`。

	基础互动协议优先使用跨形态语义，而不是绑定某一种动物动作：

	- `petting`：宿主用户摸摸来访宠物。
	- `message`：宿主用户给来访宠物留言。
	- `return_home`：宿主请求来访宠物回家。
- `pet_to_pet.greeting`：本地宠物和来访宠物互相打招呼。
- `pet_to_pet.sit_together`：来访宠物靠近本地宠物，一起安静待一会儿。
- `pet_to_pet.walk_together`：两只宠物一起移动到桌面另一处。
- Runtime 可以同时接待多只来访宠物。当前 macOS Runtime 的上限是 5 只；超过上限时宿主 Runtime 应拒绝新的敲门请求。
- 来访宠物之间也可以触发 `pet_to_pet.*` 事件。宿主 Runtime 应为参与互动的每个 visit 分别记录事件，这样每只宠物回家后都能把这段经历带回给主人。
...
移动类投影遵循统一朝向约定：生命包里的 `move` / `run` / `walk` / `hop` sprite sheet 默认面向右；宿主 Runtime 根据窗口实际移动方向决定是否水平翻转。Skill 生成素材时不能在同一个移动 sheet 中混用左右朝向，否则宠物会出现倒着跑的视觉问题。
...
`message` 表示宿主用户给来访宠物留下的一句话。当前 MVP 只记录和转发留言，不要求宠物生成回复。留言放在 `VisitEvent.data.text` 中，并在宠物回家后进入主人侧的记忆线索。
