...
- 不同动作状态之间也要保持主体视觉尺寸一致。仅仅保证每一帧都是 `64x64` 不够：睡觉、坐下、移动、招牌动作的姿态可以不同，但核心身体在画布里的占比应接近同一个参考形象，并沿用同一套锚点规则。不要出现一个动作把宠物画满画布、另一个动作只有一小团的情况。需要表现远近或戏剧性缩放时，应交给 Runtime 的位移/缩放效果处理，而不是把不同尺寸烘焙进 sprite sheet。
