...
- 原生 macOS 透明宠物窗口。
- 宠物透明 sprite 像素支持窗口级点击穿透，不再阻挡下面应用；不透明身体和气泡仍可点击/拖拽。
...
- Runtime 对 sprite sheet 做像素级 alpha 命中判断，支持镜像帧和缩放后的真实点击区域。
...
- Runtime 已新增 `AI 对话设置...` 菜单入口，支持独立配置模型，不依赖 Alma。
- 当前设置窗只保留 `DeepSeek 官方` 预设，默认 `base_url=https://api.deepseek.com/v1`、`model=deepseek-v4-flash`、`temperature=1.0`。
- API Key 存入 macOS Keychain，`~/Library/Application Support/PetY/config.json` 只保存 provider/base_url/model/temperature/api_key_ref 等非敏感字段。
- 本地宠物菜单新增 `聊天`，点击后在宠物旁边显示轻量输入条；按 Enter 发送，Esc 取消，不再使用系统弹窗。
- 聊天请求会把 Life Pack 中的宠物身份、`personality_card`、`voice.tone`、`voice.sample_lines`、`memory_rules`、最近 life log、带回记忆和最近聊天记录放入上下文。
- 聊天记录保存在 `~/Library/Application Support/PetY/Chats/<pet_id>.json`，模型返回的 `memory_candidates` 会先写入本地 life log。
- 当前聊天只支持主人和本机宠物；串门宠物聊天、动作执行和回家带回聊天记忆仍是下一步。
...
- Skill 已补充 sprite 透明背景要求：必须是真透明 PNG alpha，避免白底/棋盘格伪透明和大面积半透明阴影，保证桌面点击穿透可靠。
- 公开仓库 `xllinbupt/pet-y-public` 已建立。
- 公开 Runtime Release 已发布到 `v0.1.22`。
...
- 留言只记录和转发，暂不触发来访宠物大模型回复。
- AI 对话目前只支持本机主人和本地宠物，不支持访问别人家的宠物，也不会把串门聊天内容带回给主人。
...
2. 把本地宠物聊天继续产品化：执行动作白名单、记忆去重/重要性评分、错误提示和输入条体验。
3. 设计串门聊天：宿主使用自己的模型配置和访问宠物对话，宠物回家后给主人带回摘要和记忆候选。
4. 把好友绑定流程再产品化，减少菜单和脚本之间的割裂。
5. 为 Pet Skill 固化“访谈 -> 生图 -> 动作 sprite sheet -> 本地运行 -> 加好友”的标准流程。
...
```bash
PET_Y_RELAY=http://47.99.98.43:8787 PET_Y_LIFE_PACK=life-packs/alice-momo/pet-life.json ./scripts/run-desktop.sh
```
